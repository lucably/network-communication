...
func BuscarUsuarios(w http.ResponseWriter, r *http.Request) {
	// utilizando Query parametros, ficando assim: exp: localhost:5000/usuarios?usuario=cam
...
func AtualizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	//Caso o usuarioId for dif do usuarioID do token da erro
	if usuarioID != usuarioIDNoToken {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possivel editar um usuário que não seja o seu"))
		return
	}
	...
	etapaEdicao := "edicao"
	if erro = usuario.Preparar(etapaEdicao); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
	...
	// Passou um "nil", pois quando se edita costuma nao devolver nenhuma resposta, somente o status.
...
// PararDeSeguirUsuario permite que um usuario deixa de seguir outro
...
// BuscarSeguidores permite buscar os seguidores do usuario
...
// BuscarSeguindo permite buscar quem o usuario está seguindo
...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	if erro = seguranca.VerificarSenha(senhaSalvaNoBanco, senha.Atual); erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, errors.New("Senha Atual nao condiz com a senha do banco"))
		return
	}
